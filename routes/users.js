...
usersRouter.post('/api/auth/checkCredentials', (req, res) => {
  const { hashPassword, email } = req.body;
  let validationErrors = null;
  User.findByEmail(hashPassword)
    .then((hashedPassword, email) => {
      if (hashedPassword) return Promise.reject('OK');
      validationErrors = User.validate(req.body);
      if (validationErrors) return Promise.reject('INVALID_DATA');
      return User.create(req.body);
    })
    ...
    .catch((err) => {
      console.error(err);
      if (err === 'DUPLICATE_EMAIL')
        res.status(200).json({ message: 'You are correctly authenticated !' });
      else if (err === 'INVALID_DATA')
